...
GUIDANCE_ABLATION_MODES = frozenset({"none", "hidden_guidance", "shuffled_guidance"})


def _guidance_disabled(config) -> bool:
    return str(getattr(config, "guidance_mode", "")) == "disabled"


def _guidance_ablation_enabled(config) -> bool:
    return not _guidance_disabled(config) and str(getattr(config, "guidance_ablation_mode", "none")) != "none"
...
class GuidedXVLATrainConfig:
    ...
    def __post_init__(self) -> None:
        ...
        self.guidance_ablation_mode = str(self.guidance_ablation_mode).strip().lower()
        self.guidance_training_schedule = str(self.guidance_training_schedule).strip().lower()
        if _guidance_disabled(self):
        ...
        if self.guidance_ablation_mode not in GUIDANCE_ABLATION_MODES: raise ValueError(f"guidance_ablation_mode must be one of: {', '.join(sorted(GUIDANCE_ABLATION_MODES))}.")
...
def _load_guidance_task_config(config: GuidedXVLATrainConfig):
    if _guidance_disabled(config): return None
...
def _load_decoder_init(model, config: GuidedXVLATrainConfig) -> None:
    if _guidance_disabled(config): return
    state = load_decoder_state(_resolve_guidance_decoder_state_path(config.decoder_init_path, config.guidance_expert_type))
...
def _legacy_decoder_trainable(config: GuidedXVLATrainConfig, step: int) -> bool:
    if _guidance_disabled(config): return False
...
def _effective_phase2_expert_loss_weight(config: GuidedXVLATrainConfig) -> float:
    if _guidance_disabled(config): return 0.0
...
def _guidance_schedule_state(config: GuidedXVLATrainConfig, step: int) -> GuidedScheduleState:
    if _guidance_disabled(config): return GuidedScheduleState(phase=0, phase_name="disabled", action_loss_weight=float(config.action_loss_weight), expert_loss_weight=0.0, decoder_trainable=False, corruption_enabled=False)
...
def _step_guided_optimizer(optimizer: torch.optim.Optimizer | JointTrainingState, schedule: GuidedScheduleState, policy_scheduler=None) -> None:
    if isinstance(optimizer, JointTrainingState) and int(schedule.phase) == 1:
        optimizer.decoder_optimizer.step()
        return
    step_optimizer(optimizer)
    if not isinstance(optimizer, JointTrainingState) and policy_scheduler is not None: policy_scheduler.step()
...
def _init_guided_policy_from_base(runtime: XVLARuntime, config: GuidedXVLATrainConfig, task_cfg):
    ...
    if _guidance_disabled(config):
        _configure_explicit_stage_trainability(runtime.policy, config)
...
def build_optimizer(config: GuidedXVLATrainConfig, policy) -> torch.optim.Optimizer | JointTrainingState:
    if _guidance_disabled(config) or not hasattr(policy.model, "guidance_decoder"):
...
def _log_gpu_model_residency(policy, *, step: int, rank: int, wandb_run=None) -> None:
    ...
    if hasattr(policy.model, "guidance_decoder"): modules["policy.model.guidance_decoder"] = policy.model.guidance_decoder
...
def _trainability_metrics(policy, optimizer: torch.optim.Optimizer | JointTrainingState) -> dict[str, float]:
    guidance_decoder_params = list(policy.model.guidance_decoder.parameters()) if hasattr(policy.model, "guidance_decoder") else []
    ...
    policy_optimizer_count = float(_optimizer_parameter_count(optimizer.policy_optimizer)) if isinstance(optimizer, JointTrainingState) else float(_optimizer_parameter_count(optimizer))
    decoder_optimizer_count = float(_optimizer_parameter_count(optimizer.decoder_optimizer)) if isinstance(optimizer, JointTrainingState) and guidance_decoder_params else 0.0
    return {
        ...
        "params_policy_optimizer": policy_optimizer_count,
        "params_decoder_optimizer": decoder_optimizer_count,
    }
...
def _apply_guidance_ablation(guidance_tokens_used: torch.Tensor, guidance_available: torch.Tensor, config: GuidedXVLATrainConfig, *, mode: str) -> tuple[torch.Tensor, torch.Tensor, dict[str, float]]:
    ablation_mode = str(config.guidance_ablation_mode)
    if ablation_mode == "none" or mode == "disabled" or _guidance_disabled(config): return guidance_tokens_used, guidance_available, {"guidance_ablation_applied": 0.0, "guidance_ablation_hidden_fallback": 0.0}
    hidden_metrics = {"guidance_ablation_applied": 1.0, "guidance_ablation_hidden_fallback": 1.0}
    if ablation_mode == "hidden_guidance": return torch.zeros_like(guidance_tokens_used), torch.zeros_like(guidance_available), hidden_metrics
    if guidance_tokens_used.shape[0] <= 1: return torch.zeros_like(guidance_tokens_used), torch.zeros_like(guidance_available), hidden_metrics
    permutation = torch.roll(torch.arange(guidance_tokens_used.shape[0], device=guidance_tokens_used.device), shifts=1)
    return guidance_tokens_used[permutation], guidance_available[permutation], {"guidance_ablation_applied": 1.0, "guidance_ablation_hidden_fallback": 0.0}
...
def _prepare_guided_transformer_inputs(policy, processed_batch: dict[str, Any], inputs: dict[str, torch.Tensor], enc: dict[str, torch.Tensor], *, config: GuidedXVLATrainConfig, guidance_mode: str, corruption_enabled: bool = True) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, dict[str, torch.Tensor], torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, dict[str, float]]:
    ...
    transformer_dtype = _transformer_dtype(policy.model.transformer, action_noisy_m)
    ...
    guidance_tokens_used, guidance_available, ablation_stats = _apply_guidance_ablation(guidance_tokens_used, guidance_available, config, mode=guidance_mode)
    return targets, t, action_noisy_m, enc, guidance_tokens, guidance_tokens_used, guidance_available, proprio_m, ablation_stats
...
def compute_guided_action_loss_from_encoder(policy, processed_batch: dict[str, Any], inputs: dict[str, torch.Tensor], enc: dict[str, torch.Tensor], *, config: GuidedXVLATrainConfig, guidance_mode: str = "train", corruption_enabled: bool = True) -> tuple[torch.Tensor, dict[str, float], torch.Tensor]:
    targets, t, action_noisy_m, enc, guidance_tokens, guidance_tokens_used, guidance_available, proprio_m, ablation_stats = _prepare_guided_transformer_inputs(policy, processed_batch, inputs, enc, config=config, guidance_mode=guidance_mode, corruption_enabled=corruption_enabled)
    ...
    stats.update(ablation_stats)
    return action_loss, stats, guidance_tokens


def compute_plain_action_loss(policy, processed_batch: dict[str, Any]) -> tuple[torch.Tensor, dict[str, float]]:
    loss, log_dict = policy(processed_batch)
    action_stats = {key: float(value) for key, value in log_dict.items() if key.endswith("_loss")}
    action_stats["action_total"] = float(log_dict["loss"])
    return loss, action_stats


def _grad_norm(policy, prefix: str) -> float:
    total = 0.0
    for name, parameter in policy.named_parameters():
        if not name.startswith(prefix) or parameter.grad is None: continue
        grad = parameter.grad.detach()
        total += float(torch.sum(grad * grad).item())
    return total ** 0.5


def _optional_scalar(module, attr_name: str) -> float | None:
    value = getattr(module, attr_name, None)
    if value is None: return None
    if torch.is_tensor(value): return float(value.detach().item())
    return float(value)
...
def _concat_gate_stats(transformer, native_context: torch.Tensor, guidance_tokens: torch.Tensor) -> dict[str, float]:
    ...
    return {"concat_gate_mean": float(gate.mean().item()), "concat_gate_min": float(gate.min().item()), "concat_gate_max": float(gate.max().item())}


@torch.no_grad()
def collect_guidance_debug_metrics(policy, processed_batch: dict[str, Any], config: GuidedXVLATrainConfig) -> dict[str, float]:
    was_training = policy.training
    vlm_was_training = policy.model.vlm.training
    guidance_decoder_was_training = policy.model.guidance_decoder.training
    policy.eval()
    ...
    targets, t, action_noisy_m, enc, guidance_tokens, guidance_tokens_used, _, proprio_m, _ = _prepare_guided_transformer_inputs(policy, processed_batch, inputs, enc, config=config, guidance_mode="guided")
    ...
    pred_zero = transformer(domain_id=inputs["domain_id"], action_with_noise=action_noisy_m, t=t, proprio=proprio_m, guidance_tokens=torch.zeros_like(guidance_tokens), guidance_available=guidance_available_disabled, **enc)
    metrics = {
        "guidance_pred_delta_mean_abs": float((pred_guided - pred_zero).abs().mean().item()),
        "guidance_pred_delta_max_abs": float((pred_guided - pred_zero).abs().max().item()),
        "guidance_tokens_mean_abs": float(guidance_tokens.abs().mean().item()),
        "grad_norm_guidance_decoder": _grad_norm(policy, "model.guidance_decoder."),
        "grad_norm_guidance_proj": _grad_norm(policy, "model.transformer.guidance_proj."),
        "grad_norm_cross_attn": _grad_norm(policy, "model.transformer.cross_attn."),
        "grad_norm_cross_attn_query_norm": _grad_norm(policy, "model.transformer.cross_attn_query_norm."),
        "grad_norm_cross_attn_guidance_norm": _grad_norm(policy, "model.transformer.cross_attn_guidance_norm."),
        "grad_norm_concat_gate": _grad_norm(policy, "model.transformer.concat_gate."),
    }
    metrics.update(_concat_gate_stats(transformer, torch.cat([action_proj, z_proj, aux_proj], dim=1), guidance_proj))
    cross_attn_gamma = _optional_scalar(transformer, "cross_attn_gamma")
    if cross_attn_gamma is not None: metrics["cross_attn_gamma"] = cross_attn_gamma
    if cross_attn_gamma is not None: metrics["cross_attn_gain"] = float(math.tanh(cross_attn_gamma))
    if was_training:
        policy.train()
        if not vlm_was_training: policy.model.vlm.eval()
        if not guidance_decoder_was_training: policy.model.guidance_decoder.eval()
    return metrics
...
def run_validation(config: GuidedXVLATrainConfig, runtime: XVLARuntime, policy, guidance_source, val_loader: DataLoader, prefix: str = "val", guidance_mode: str = "guided", schedule: GuidedScheduleState | None = None) -> dict[str, float]:
    ...
    for raw_batch in val_loader:
        ...
        if _guidance_disabled(config):
            action_loss, action_stats = compute_plain_action_loss(policy, processed_batch)
...
def restore_guided_policy_checkpoint(config: GuidedXVLATrainConfig, runtime: XVLARuntime, policy, checkpoint_dir: str | Path) -> None:
    ...
    if _guidance_disabled(config):
        restored_policy = XVLAPolicy.from_pretrained(str(checkpoint_dir), config=policy.config)
...
class GuidedTrainingModule(nn.Module):
    ...
    def forward(self, processed_batch: dict[str, Any], target, *, step: int | None = None, rank: int = 0, wandb_run=None, schedule: GuidedScheduleState | None = None):
        if _guidance_disabled(self._config):
            action_loss, action_stats = compute_plain_action_loss(self.policy, processed_batch)
            ...
            return action_weight * action_loss, action_stats, expert_stats
...
def _validation_metrics(config: GuidedXVLATrainConfig, runtime: XVLARuntime, policy, guidance_source, val_loader: DataLoader, *, step: int) -> dict[str, float]:
    ...
    if bool(config.validation_include_no_guidance) and not _guidance_disabled(config): metrics.update(run_validation(config, runtime, policy, guidance_source, val_loader, prefix="val_no_guidance", guidance_mode="disabled", schedule=schedule))
    return metrics
...
def train_guided_xvla(config: GuidedXVLATrainConfig) -> None:
    ...
    if isinstance(optimizer, JointTrainingState):
        ...
        plain_policy_scheduler = accelerator.unwrap_model(train_module).policy.config.get_scheduler_preset().build(optimizer, config.steps)
    ...
    if hasattr(accelerator.unwrap_model(train_module).policy.model, "set_guidance_trainability"): accelerator.unwrap_model(train_module).policy.model.set_guidance_trainability(step)
    ...
    if is_main and step == 0 and val_loader is not None and should_run_validation_step(0, config.steps, config.validation_freq, emitted_validation_steps):
        val_metrics = {"event": "validation_step", "step": 0, "guidance_ablation_mode": str(config.guidance_ablation_mode), **_validation_metrics(config, runtime, runtime.policy, guidance_source, val_loader, step=0)}
    ...
    while step < config.steps:
        for raw_batch in loader:
            ...
            if hasattr(accelerator.unwrap_model(train_module).policy.model, "set_guidance_trainability"): accelerator.unwrap_model(train_module).policy.model.set_guidance_trainability(current_step)
            processed_batch = preprocess_batch(runtime, raw_batch)
            target = None if _guidance_disabled(config) else load_guidance_target(guidance_source, raw_batch, config, runtime.teacher_image_key)
            with accelerator.accumulate(train_module):
                ...
                should_debug = is_main and not _guidance_disabled(config) and debug_every > 0 and current_step % debug_every == 0
                if should_debug:
                    debug_metrics = {"event": "guidance_debug", "step": int(current_step), **collect_guidance_debug_metrics(accelerator.unwrap_model(train_module).policy, processed_batch, config)}
                ...
                _step_guided_optimizer(optimizer, schedule, policy_scheduler=plain_policy_scheduler)
                zero_optimizer_grad(optimizer)
            step = current_step
            if is_main and step % max(int(config.log_every), 1) == 0:
                metrics = {
                    ...
                    "guidance_ablation_mode": str(config.guidance_ablation_mode),
            ...
            if is_main and val_loader is not None and should_run_validation_step(step, config.steps, config.validation_freq, emitted_validation_steps):
                val_metrics = {"event": "validation_step", "step": int(step), "guidance_ablation_mode": str(config.guidance_ablation_mode), **_validation_metrics(config, runtime, policy, guidance_source, val_loader, step=int(step))}
                print(json.dumps(val_metrics))
