...
def _load_episode_tables(root: Path) -> tuple[dict[Path, pd.DataFrame], dict[int, tuple[Path, int]]]:
    ...
    for parquet_path in sorted((root / "meta" / "episodes").rglob("*.parquet")):
        ...
        for row_idx, ep_idx in enumerate(df["episode_index"].astype(int).tolist()): row_locs[int(ep_idx)] = (parquet_path, int(row_idx))
...
def _video_keys(root: Path) -> list[str]:
    with open(root / "meta" / "info.json") as f: info = json.load(f)
...
def repair_video_timestamps(root: Path, validate_loader: bool) -> None:
    root = root.resolve()
    fps, video_keys = _fps(root), _video_keys(root)
    if not video_keys: raise ValueError(f"No video keys found in {root / 'meta' / 'info.json'}")
    tables, row_locs, changed = _load_episode_tables(root), None, 0
    tables, row_locs = tables
    for video_key in video_keys:
        ...
        for (chunk_idx, file_idx), episode_ids in sorted(_episode_groups(tables, video_key).items()):
            ordered_episode_ids, cursor = _sort_episode_ids(tables, episode_ids, video_key), 0
            for ep_idx in ordered_episode_ids:
                table_path, row_idx = row_locs[ep_idx]
                df = tables[table_path]
                length = int(df.at[row_idx, "length"])
                expected_from, expected_to = cursor / fps, (cursor + length) / fps
                old_from, old_to = float(df.at[row_idx, from_col]), float(df.at[row_idx, to_col])
            ...
            if actual_frames != cursor: raise RuntimeError(f"{video_path} frame count mismatch: decoded={actual_frames}, expected={cursor}")
    for parquet_path, df in tables.items(): df.to_parquet(parquet_path, index=False)
    print(f"Repaired {changed} episode video timestamp row(s) under {root}")
    if not validate_loader: return
    repo_src = Path('/home/jose/EMAI-Thesis/repos/lerobot/src')
    if repo_src.exists() and str(repo_src) not in sys.path: sys.path.insert(0, str(repo_src))
